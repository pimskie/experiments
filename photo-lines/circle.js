...
const drawSegment = (imageData, line, ctxOutput, iteration) => {
	...
	const intensity = getPixelIntensity(imageData, line.posTo);
	const noiseValue = 1 + (simplex.noise2D(posFrom.x * noiseScale, posFrom.y * noiseScale, iteration * 0.001));
	const lineIntensity = noiseValue;

	line.update(lineIntensity);

	ctxOutput.beginPath();
	ctxOutput.lineWidth = noiseValue;
