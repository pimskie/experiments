...
// const planets = headers.filter(h => h.textContent.length > 2).map((header, index) => {
// 	const headerIndex = index + 1;
// 	const planet = {};

// 	const nameLower = header.textContent.toLowerCase().trim();
// 	const name = nameLower.charAt(0).toUpperCase() + nameLower.slice(1);

// 	planet.name = name;

// 	const properties = rows.forEach((row, rowIndex) => {
// 		let property = row.querySelector('td').textContent;

// 		property = property.replace(/ /g, '').replace('?', '');
// 		property = property.charAt(0).toLowerCase() + property.slice(1);

// 		const value = parseFloat(row.querySelector(`td:nth-child(${headerIndex + 1})`).textContent) || 0;
