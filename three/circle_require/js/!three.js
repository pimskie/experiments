...
THREE_APP.prototype = {
    ...
    selectArtist: function( artist ) {       
        ...
        if (  this.artistTree[artist.id] == undefined) {                      
            ...
            if ( ! artist.data.bio) {
                disco_app.searchArtist( artist.data.name );
                console.log('load bio');
            } 

            if ( ! artist.data.similarLaded) {
                disco_app.searchSimilar( artist.data.name );
                console.log('load similar');
            } else {
                this.addSimilar( artist.data.similar );
            }
        ...
        $('#artist_image').attr('src', this.artistSelected.data.image[3]['#text']);
    ...
    artistDataLoaded: function( data ) {
        this.artistSelected.data = data.artist;
        $('.sidebar .bio .content').show().html(data.artist.bio.content.replace(/\n/g, '<br />'))
            .find('a').attr('target', '_blank')
            .parent().find('a.bbcode_artist').css('border', '1px solid red').click(function(e){
                e.preventDefault();
    ...
    artistVideoLoaded: function( data ) {
        this.artistSelected.videos = data;
	...
	addSimilar: function(artists) {	
       ...
       this.artistSelected.data.similar = artists;
       this.artistSelected.data.similarLoaded = true;
	   ...
	   _.each(artists, function(artist) {
	       ...
	       position.z -= 10;
			...
			var cube = me.createArtist( artist, to, dest );
			cube.angle = angle;
